...
test("guest cart and customer address workflows are functional", async ({
  ...
  const guestItem = await pool.query<{ variantId: string }>(
    `SELECT "variantId" FROM "CartItem" WHERE "cartId" = $1 LIMIT 1`,
    [cartId],
  );
  variantId = guestItem.rows[0]?.variantId ?? "";
  const customerItem = await pool.query<{ quantity: number }>(
    `SELECT ci.quantity
     FROM "CartItem" ci
  ...
  await expect(
    page.locator('header a[href="/cart"]:visible'),
  ).not.toContainText("Cart0");
